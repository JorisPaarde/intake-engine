...
- Airco-templatebron: `database/data/templates/airco/v1.php` … latest `v16.php` (wijziging = nieuwe templateversie, ADR-0001). V13: `free_group_known` pas ná `fusebox_photo` (show `filled`); AI-prefill slaat de ja/nee over. V14: korte kruipruimte-/L×B×H-labels. V15: `dormer` (dakkapel) bij `outdoor_location` voor catalogusprefill — niet terugschrijven in gepubliceerde v12–v14. V16: `room_area_m2` (L×B óf trusted m²; hoogte apart, BL-101). Demo allowlist/short-keys: zie BL-078 (niet hier wijzigen).
...
- **Publieke demo (BL-001/BL-051 + UX BL-066–071/073/075/076/078 + BL-091/095):** start provisiont alleen ephemeral tenant/user → dashboard; intake via normale create (inclusief `EnrichIntakeAddress` + `DeriveIntentFromRequest`). Postcode/huisnummer én klantnaam zijn leeg — installateur typt zelf; `DEMO_ADDRESS_*` / `DEMO_CUSTOMER_NAME` is alleen tiptekst/placeholder (geen Voorbeeldklant-prefill). Sample-dossier-CTA alleen vóór echte werkplek-inhoud (plekken/foto’s/opstelling); sample-load overschrijft geen live `PDOK Luchtfoto RGB` (BL-075). Sticky toont **met inhoud** los van klaar-voor-offerte. AI short-circuiteert niet op `is_demo`; klantmail wel. PDF-aanvraag: `POST demo.report-pdf` → lead + PDF-mail (`PRODUCT_INTEREST_MAIL_TO`, default `info@jpwebcreation.nl`). Sessiesleutels: `public_demo_mode`, `public_demo_intake_id` (pas na create), `public_demo_path_chosen`, `public_demo_guide_step`. Klantpad = **volledige** airco-klantwizard (geen allowlist/`public_demo_short_customer`; BL-078). Rolkeuze: `POST demo.path.choose` (installer-first CTA). Sample-dossier: `POST demo.scenario.load` (optioneel). Middleware allowlist dekt show+workspace-dossieracties (BL-091); blokkeert tweede create en metrics/profiel/dev. **Demo beëindigen**/TTL/stale auth → `/demo/beeindigd` (niet framework-404 / niet echt `/login`); `demo.ended` + echte login wissen `url.intended` + demo-flags zodat je niet op een dode demo-URL landt. Coach: alleen welkom + branch; welkom (BL-095) = productuitleg in `resources/js/demo-guide.js` (**Zo werkt de Digitale Opname**), geen 6-staps stack. Klantpad-banner (`demo-scope-notice`, BL-076): korte installateurstaal (**Demo — wat de klant ziet**), geen featuresheet “Wel aan / Bewust uitgeschakeld”.
- **Staging-inzage:** de dev-admin op `/dev` (BL-028, ADR-0008) toont per opname de ruwe binnengekomen data (externe feiten, AI-runs, antwoorden, uploads, activity-events) en een passieve dienststatus — sneller dan zelf via DB/logs speuren. Tab **AI-invoer testen** (`/dev/ai-input-test`, BL-104) dry-runt openingszin → lokale parser → catalogus-AI via `EvaluateRequestIntent` (geen intakes/AI-runs/mail). Alleen aan op local/staging (`config('devadmin.enabled')`), hard 404 op production.
...
- Demo-user `installateur@example.com` ontbreekt op staging (deploy seedt alleen templates) → registreren als fallback. **Start demo** staat standaard aan voor gasten (`DEMO_ENABLED` default `true`); verborgen voor ingelogde users; alleen uit bij expliciet `DEMO_ENABLED=false` in `shared/.env` (niet in git).
