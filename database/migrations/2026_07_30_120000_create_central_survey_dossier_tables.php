...
{
    private const OPTION_PLACEMENT_INSTALLATION_FK = 'airco_iop_installation_fk';

    private const OPTION_PLACEMENT_PLACEMENT_FK = 'airco_iop_placement_fk';

    public function up(): void
    {
        $workflowModeMissing = ! Schema::hasColumn('intakes', 'workflow_mode');
        $customerAccessMissing = ! Schema::hasColumn('intakes', 'customer_access_enabled');

        if ($workflowModeMissing || $customerAccessMissing) {
            Schema::table('intakes', function (Blueprint $table) use ($workflowModeMissing, $customerAccessMissing): void {
                if ($workflowModeMissing) {
                    $table->string('workflow_mode')->default('customer')->after('status');
                }

                if ($customerAccessMissing) {
                    $table->boolean('customer_access_enabled')->default(true)->after('access_token');
                }
            });
        }

        $analysisPathMissing = ! Schema::hasColumn('intake_uploads', 'analysis_path');
        $analysisMimeMissing = ! Schema::hasColumn('intake_uploads', 'analysis_mime_type');
        $analysisSizeMissing = ! Schema::hasColumn('intake_uploads', 'analysis_size_bytes');
        $analysisChecksumMissing = ! Schema::hasColumn('intake_uploads', 'analysis_checksum');

        if ($analysisPathMissing || $analysisMimeMissing || $analysisSizeMissing || $analysisChecksumMissing) {
            Schema::table('intake_uploads', function (Blueprint $table) use (
                $analysisPathMissing,
                $analysisMimeMissing,
                $analysisSizeMissing,
                $analysisChecksumMissing,
            ): void {
                if ($analysisPathMissing) {
                    $table->string('analysis_path')->nullable()->after('path');
                }

                if ($analysisMimeMissing) {
                    $table->string('analysis_mime_type')->nullable()->after('analysis_path');
                }

                if ($analysisSizeMissing) {
                    $table->unsignedBigInteger('analysis_size_bytes')->nullable()->after('analysis_mime_type');
                }

                if ($analysisChecksumMissing) {
                    $table->string('analysis_checksum')->nullable()->after('analysis_size_bytes');
                }
            });
        }

        $purposeMissing = ! Schema::hasColumn('intake_follow_up_rounds', 'purpose');
        $returnStatusMissing = ! Schema::hasColumn('intake_follow_up_rounds', 'return_status');

        if ($purposeMissing || $returnStatusMissing) {
            Schema::table('intake_follow_up_rounds', function (Blueprint $table) use ($purposeMissing, $returnStatusMissing): void {
                if ($purposeMissing) {
                    $table->string('purpose')->default('follow_up')->after('round_number');
                }

                if ($returnStatusMissing) {
                    $table->string('return_status')->nullable()->after('status');
                }
            });
        }

        if (! Schema::hasTable('dossier_subjects')) {
        ...
        if (! Schema::hasTable('dossier_records')) {
        ...
        if (! Schema::hasTable('dossier_evidence_links')) {
        ...
        if (! Schema::hasTable('dossier_decision_areas')) {
        ...
        if (! Schema::hasTable('contribution_tasks')) {
        ...
        if (! Schema::hasTable('airco_placement_options')) {
        ...
        if (! Schema::hasTable('airco_installation_options')) {
        ...
        if (! Schema::hasTable('airco_installation_option_placements')) {
            Schema::create('airco_installation_option_placements', function (Blueprint $table): void {
                $table->id();
                $table->unsignedBigInteger('airco_installation_option_id');
                $table->unsignedBigInteger('airco_placement_option_id');
                $table->string('role');
                $table->unsignedSmallInteger('sort_order')->default(0);
                $table->timestamps();
                ...
                $table->foreign(
                    ...
                    self::OPTION_PLACEMENT_INSTALLATION_FK,
                )->references('id')->on('airco_installation_options')->cascadeOnDelete();
                $table->foreign(
                    'airco_placement_option_id',
                    self::OPTION_PLACEMENT_PLACEMENT_FK,
                )->references('id')->on('airco_placement_options')->cascadeOnDelete();
                $table->unique(
        ...
        if (! Schema::hasTable('airco_connections')) {
        ...
        if (! Schema::hasColumn('pipe_route_sessions', 'airco_connection_id')) {
        ...
        if (! Schema::hasTable('installation_outcomes')) {
    ...
    public function down(): void
    {
        ...
        if (Schema::hasTable('pipe_route_sessions')
            && Schema::hasColumn('pipe_route_sessions', 'airco_connection_id')) {
            Schema::table('pipe_route_sessions', function (Blueprint $table): void {
        ...
        $this->dropColumnsIfPresent('intake_follow_up_rounds', ['purpose', 'return_status']);
        $this->dropColumnsIfPresent('intake_uploads', [
            'analysis_path',
            'analysis_mime_type',
            'analysis_size_bytes',
            'analysis_checksum',
        ]);
        $this->dropColumnsIfPresent('intakes', ['workflow_mode', 'customer_access_enabled']);
    }
        ...
        foreach ($sources as $table => $type) {
            DB::table($table)
                ...
                ->chunkById(250, function ($rows) use ($intakeId, $companyId, $rootId, $type): void {
                    foreach ($rows as $row) {
                        $existingLink = DB::table('dossier_evidence_links')
                            ->where('intake_id', $intakeId)
                            ->where('company_id', $companyId)
                            ->where('dossier_subject_id', $rootId)
                            ->whereNull('dossier_record_id')
