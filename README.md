...
De publieke homepage is een installateursfunnel met fictieve productweergaven, interactieve demo en een zelfstandige, privacybegrensde interesse-CTA, visueel afgestemd op de JPWebcreation-huisstijl; sinds BL-107 volgt de ingelogde app dezelfde productstijl (salie/bosgroen, oker, terracotta, Inter) met tenantkleur voor primaire acties. Contactinzendingen starten nooit een technische opname. Gebruikersgerichte app-teksten volgen gecontroleerd eenvoudig Nederlands ([docs/language.md](docs/language.md)); Nieuwe airco-opnames gebruiken template **v16** (vloeroppervlak L×B of m² + hoogte apart, BL-101; dakkapel/`dormer` uit v15; korte kruipruimte-/matenlabels uit v14; meterkastfoto vóór vrije-groepvraag uit v13; offerte-kritisch bewijs uit v12; BL-063/064/074/077/082/101).

**Open operationeel werk:** de begeleidde BL-001-demo (installateursstart + rolkeuze + coachmarks) is codegereed maar blijft `in_progress` tot de staging-/mobiele smoke is uitgevoerd. `MEDIA_DISK=s3` is app-ondersteund (BL-013); omschakelen is env-only. Externe AI-activering is env-only (provider/key/featurevlaggen/budgetcaps); SMTP en overige host/env-acties staan in [docs/DEPLOYMENT.md § Handmatige acties](docs/DEPLOYMENT.md#handmatige-acties-producteigenaar). Visuele browser-QA: [docs/browser-test-flow.md](docs/browser-test-flow.md). Uitslag: [docs/functional-test-status.md](docs/functional-test-status.md).
