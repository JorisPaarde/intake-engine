...
- **Begeleide leidingroute — backend-slice** (BL-029, ADR-0009). Nieuwe, stateful capaciteit die de leidingroute tussen binnen- en buitenunit begeleid vastlegt: per foto beoordeelt AI of de wand/doorvoer zichtbaar is en of een route naar buiten aannemelijk is, koppelt elke foto aan een routesegment, vraagt steeds om één gerichte vervolgfoto, en vat de segmenten samen tot een voorgestelde + alternatieve route met onzekerheden en ontbrekende controles. Beide AI-stappen leveren gestructureerde JSON met float-`confidence`. **Modeltiering, los van het globale `ai.model`:** `config('ai.route.model')` (default `gpt-5.6-terra`) doet de standaardanalyse; bij lage zekerheid of een niet-doorlopende route escaleert de synthese naar `config('ai.route.review_model')` (default `gpt-5.6-sol`). Model-ID's zijn env-overschrijfbaar (`AI_ROUTE_MODEL`/`AI_ROUTE_REVIEW_MODEL`); de AI-laag kreeg daarvoor een per-call `model`-override. De installateur keurt de route altijd zelf goed (`ApprovePipeRoute`). Draait parallel naast de bestaande `pipe_route`-sectie, staat achter `AI_ROUTE_ANALYSIS_ENABLED` (standaard uit) en is soft-fail. Meer beeld naar een externe LLM valt onder de DPIA-voorwaarde (ADR-0005/0009). De **installateur-goedkeuringsweergave** staat op de opname-detailpagina (voorgestelde/alternatieve route, onzekerheden, ontbrekende controles en segmentfoto's, plus Goedkeuren/Afkeuren via `intakes.pipe-route.review`). De **klant-wizard** (`/o/{token}/leidingroute`, Livewire `PipeRouteWizard`) laat de klant foto-voor-foto de route vastleggen: elke foto wordt beoordeeld, de app toont steeds de volgende gerichte foto-instructie, en sluit af met de samengevatte route. Routefoto's worden opgeslagen via `StoreRouteSegmentPhoto` (los van de template-vragen, met dezelfde normalisatie).
