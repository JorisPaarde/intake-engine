...
| — | BL-066 | Demo beëindigen: bevestiging + verlopen-pagina | E5 | done | high | A · product/demo/UX |
| — | BL-067 | Demo-rolkeuze: installateur primair | E5 | done | high | A · product/demo/UX |
| — | BL-068 | Demo-create: geen ‘mailen’ als mail uit staat | E5 | done | high | A · product/demo/UX |
...
- **Resultaat:** overzicht toont **Klanttaak: X% beantwoord** naast **Klaar voor offerte: Y/Z** (+ volgende actie); dashboardkolom hernoemd naar Klanttaak; geen `% compleet` meer als opname-waarheid.
...
- **Resultaat:** ADR-0013; `PrefillAnswersFromKnownContext` + `request-prefill-v1`; airco **v15** (`dormer`); lokale parser alleen offline-fallback zonder outdoor-heuristiek.
...
- **Resultaat:** confirm op alle end-demo-knoppen; TTL/logout → `/demo/beeindigd`; Nederlandse `errors/404`.
...
- **Resultaat:** **Zelf de opname doen** is indigo-primair; **Bekijk wat de klant ziet** is outline-secundair.
...
- **Resultaat:** demo-knop **Opname aanmaken**; JS overschrijft die niet meer; flash/branch zonder ‘mailen we’.
...
- **Resultaat:** alleen welkom + rolkeuze-modal; geen Stap N/6; sample-CTA **Optioneel: toon voorbeelddossier**.
...
- **Resultaat:** `APP_NAME=Digitale Opname`; **Opname openen**; geen `· vN` in overzicht; AI-gebiedslabels + confidencePhrase; klant/demo **u**; NL-validatie in `app.js`; NL 404/ended.
...
### BL-081 — Werkplek-copy: binnen-/buitenunit i.p.v. plekken/posities

- **Status:** done · **Prioriteit:** high · **Datum:** 2026-09-01 · **PR:** #86 · **Epic:** E7 · **Band:** O · **Afhankelijk:** BL-053/059
- **Aanleiding:** producteigenaar: “Plekken en posities toevoegen” / “Mogelijke plekken” is vaag; installateurs spreken over binnenunit en buitenunit (enum-labels zijn al Binnenunit, Buitenunit, Stroomaansluiting, Afvoerpunt).
- **Doel:** herschrijf installateurs-UI-chrome (geen domain keys): sectiekop, toevoegen/sticky CTA, labels, bodycopy zonder “plekken”/“posities”; typekeuze als vier altijd zichtbare radio’s i.p.v. dropdown “Soort positie”.
...
- **Status:** done · **Datum:** 2026-09-03 · **PR:** #91 · **Prioriteit:** high · **Epic:** E6 · **Band:** detailpagina-UX
...
- **Acceptatie:** vanaf de detailpagina is met één klik de eerstvolgende open punt te openen op de exacte plek in de werkplek; elke gemarkeerde blocker is los aanklikbaar; `composer check` groen.
- **Hypothese:** UI + deep-links op bestaande readiness-data; geen modelwijziging.
...
- **Hypothese:** UI-conditielogica + optionele bestaande trigger; geen modelwijziging.

### BL-086 — Opnamedetailpagina herstructureren rond "wat nu te doen"

- **Status:** done · **Datum:** 2026-09-03 · **PR:** #91 · **Prioriteit:** medium · **Epic:** E6 · **Band:** detailpagina-UX
...
- **Status:** done · **Datum:** 2026-09-03 · **PR:** #91 · **Prioriteit:** medium · **Epic:** E7 · **Band:** detailpagina-UX
...
- **Status:** done · **Datum:** 2026-09-03 · **PR:** #91 · **Prioriteit:** low · **Epic:** E6 · **Band:** detailpagina-UX
...
- **Status:** done · **Prioriteit:** high · **Datum:** 2026-09-06 · **PR:** #102 · **Epic:** E6 · **Band:** opname-UX · **Volgt op:** BL-053/055/056/084/086
...
- **Niet in scope:** wijzigen van de acht beslisgebieden, readinessregels of de aparte aandachtspunten op de lees-detailpagina.
- **Acceptatie:** elk dossieronderdeel komt precies één keer voor in het centrale overzicht; open, te controleren, gereed en niet-relevante onderdelen zijn vanuit hetzelfde overzicht herkenbaar; een onderdeel is volledig aanklikbaar/uitklapbaar en toont zijn blocker/detail zonder dezelfde tekst elders te herhalen; **Open punten** en **Nog N open** renderen geen tweede kaartenlijst; bestaande ankers en volgende-stapnavigatie blijven werken; desktop en circa 390 px hebben geen overflow; `SurveyWorkspaceTest` dekt beide staten (met en zonder open punten); `composer check` groen.
- **Hypothese:** informatiearchitectuur/UI op bestaande readiness-data; geen modelwijziging.
...
- **Status:** done · **Prioriteit:** high · **Datum:** 2026-09-06 · **PR:** #103 · **Epic:** E3/E7/E9 · **Band:** ruimtegegevens · **Volgt op:** BL-074/079/098
...
- **Niet in scope:** een capaciteitsberekening of vermogensadvies ontwerpen; volledige BAG-gebruiksoppervlakte als kameroppervlakte gebruiken; lage-zekerheids-AI zonder controle als waarheid opslaan.
- **Acceptatie:** per ruimte kan de oppervlaktegrondslag `length_m + width_m` of een afzonderlijk `area_m2` zijn; één geldige grondslag voorkomt de dubbele vraag en maakt het oppervlakte-deel van capaciteit compleet; conflicterende L×B en m² worden als controlepunt getoond in plaats van stil overschreven; exact AI-m² telt alleen wanneer een objectspecifieke acceptatieregel bronkwaliteit, evidence, consistentie en impact voldoende vindt; bron en zekerheid blijven zichtbaar; ontbrekende hoogte blokkeert capaciteit niet standaard en verschijnt alleen als aparte taak wanneer een expliciete regel haar nodig maakt; handmatige correctie herberekent readiness; tests dekken L×B, betrouwbaar m², lage zekerheid, conflict en hoogte-wel/niet-nodig; `composer check` groen.
- **Hypothese:** kleine uitbreiding van het ruimtedomein plus aanpassing van readiness/UI; geen wijziging van het onderscheid tussen klanttaakcompleetheid en technische beslisgereedheid.
- **Resultaat:** `RoomDimensions`/`RoomAreaAcceptance`/`RoomHeightRequirement`; capaciteit via L×B of trusted `area_m2`; conflict/lage zekerheid = controlepunt; hoogte alleen bij zolderregel; airco v16 + `request-prefill-v4`; tests + `composer check`.
...
- **Hypothese:** een pure evaluatie-/resultaatlaag kan de bestaande muterende apply-stap voorafgaan; productie en dev formatteren dan dezelfde evaluatie-uitkomst, terwijl alleen productie deze op een echte opname toepast.
- **Oplevering:** `EvaluateRequestIntent` + `RequestPrefillOutcomeClassifier` gedeeld met productie-apply; Dev-tab **AI-invoer testen**; contract- + featuretests.
...
- **Niet in scope:** readiness-/taakregels, routes verwijderen, e-mail-/PDF-opmaak, marketinghomepage.
- **Acceptatie:** desktop en ~390 px: werkplek, nieuwe opname, opnamelijst, klantwizard en login in nieuwe stijl; **Vraag de klant** maakt en verstuurt direct; deep-links openen dichtgeklapte blokken; `composer check` groen.
