...
> **Documentversie:** 1.24 · **Laatste update:** 2026-07-21 · Onderhoud: zie [AGENTS.md](../AGENTS.md)
...
| Publieke demo “Start demo” (BL-001) | todo | - | Na deploy (demo standaard aan): knop op `/` voor gasten, redirect `/o/{token}`, banner met uitgeschakelde volledige-app-stappen, bedankt-copy met dezelfde toelichting + registratielink. Ingelogd: geen demoknop. Als `shared/.env` nog `DEMO_ENABLED=false` heeft: op `true` of verwijderen + `config:cache`. |
