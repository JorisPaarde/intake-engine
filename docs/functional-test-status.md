...
| Begeleide leidingroute — installateur-UI (BL-029) | todo | - | Geautomatiseerd getest (`PipeRouteReviewTest`). Op staging: opname-detailpagina toont paneel "Leidingroute (begeleid)" met voorgestelde/alternatieve route, onzekerheden, ontbrekende controles en segmentfoto's; Goedkeuren/Afkeuren werkt en legt de beslissing vast. |
| Begeleide leidingroute — klant-wizard (BL-029) | todo | - | Geautomatiseerd getest (`PipeRouteWizardTest`). Op staging met AI aan: `/o/{token}/leidingroute` — foto toevoegen → per-foto beoordeling + volgende-foto-instructie; "Route samenvatten" toont voorgestelde route; foto's blijven bewaard als AI uit staat. |
