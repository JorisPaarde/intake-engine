...
> **Documentversie:** 2.15 · **Laatste update:** 2026-09-04 · Onderhoud: zie [AGENTS.md](../AGENTS.md)
...
- **Één AI-prefillveld op create (BL-093):** optioneel **Beschrijf wat de klant wil** (`prefill[request_reason]`) onder **AI vult de vragen in** — installateur noteert de aanvraag; AI/app (`DeriveIntentFromRequest`) vult alles in wat zeker genoeg is en laat alleen open vragen over. Geen multi-veld-prefill-UI meer. Backend kan nog andere `prefill`-keys in de create-POST accepteren (tests); die staan niet in de UI.
