...
> **Documentversie:** 2.12 · **Laatste update:** 2026-09-04 · Onderhoud: zie [AGENTS.md](../AGENTS.md)
...
- **Geen installateur-prefillblok op create (BL-093):** het optionele “Alvast invullen”-blok is verwijderd (dubbel met vragen/werkplek die daarna volgen). Backend kan nog `prefill` in de create-POST accepteren (o.a. demo `request_reason`); de create-UI toont die velden niet.
...
|-------------|------|--------|
| `installer_prefillable: true` | Optionele server-side create-prefill via `CreateIntake` (POST `prefill[...]`; o.a. demo `request_reason`). **Geen create-UI meer (BL-093)** — vragen volgen in wizard/werkplek. | Opgeslagen met `prefill_source=installer`. Airco v10 voegt `installer` aan `skip_when_prefilled_by` toe; oudere gepinde versies tonen de waarde nog bewerkbaar. Prefill zet de opname niet op `in_progress`. |
