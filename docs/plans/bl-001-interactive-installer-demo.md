...
> **Documentversie:** 2.1 · **Laatste update:** 2026-08-05 · Onderhoud: zie [AGENTS.md](../../AGENTS.md)
...
De publieke demo start als een **echte installateursopname**: tijdelijke tenant → dashboard → *Nieuwe opname* (vooringevuld) → rolkeuze i.p.v. mail → klant- of installateurspad, begeleid met pop-ups. Prospects moeten zien dat postcode/huisnummer woningdata ophaalt en dat AI foto’s en tekst kan interpreteren — die paden zijn in de demo niet uitgeschakeld.

## Verkoopverhaal

1. Welkom als tijdelijke installateur (geen account).
2. Aanmaken van een opname zoals na een bestaande aanvraag (vooringevuld voorbeeldadres met PDOK-match).
...
- Demo-create gebruikt hetzelfde `EnrichIntakeAddress` + `DeriveIntentFromRequest`-pad als productie (defaultadres: Bernadottelaan 273, Haarlem).
...
- Na opslaan: geen mail; woninggegevens/luchtfoto zichtbaar waar bronnen aan staan; modal met beide vervolgpaden.
